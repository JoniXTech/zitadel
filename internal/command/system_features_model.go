...
func (m *SystemFeaturesWriteModel) Query() *eventstore.SearchQueryBuilder {
	return eventstore.NewSearchQueryBuilder(eventstore.ColumnsEvent).
		...
		EventTypes(
			...
			feature_v2.SystemDisableUserSelfService,
		).
...
func (wm *SystemFeaturesWriteModel) setCommands(ctx context.Context, f *SystemFeatures) []eventstore.Command {
	...
	cmds = appendFeatureUpdate(ctx, cmds, aggregate, wm.DisableUserSelfService, f.DisableUserSelfService, feature_v2.SystemDisableUserSelfService)
	return cmds
}
