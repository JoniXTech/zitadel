...
func (m *InstanceFeaturesWriteModel) Query() *eventstore.SearchQueryBuilder {
	return eventstore.NewSearchQueryBuilder(eventstore.ColumnsEvent).
		...
		EventTypes(
			...
			feature_v2.InstanceDisableUserSelfService,
		).
...
func reduceInstanceFeature(features *InstanceFeatures, key feature.Key, value any) {
	...
	case feature.KeyDisableUserSelfService:
		v := value.(bool)
		features.DisableUserSelfService = &v
	}
}
...
func (wm *InstanceFeaturesWriteModel) setCommands(ctx context.Context, f *InstanceFeatures) []eventstore.Command {
	...
	cmds = appendFeatureUpdate(ctx, cmds, aggregate, wm.DisableUserSelfService, f.DisableUserSelfService, feature_v2.InstanceDisableUserSelfService)
	return cmds
}
